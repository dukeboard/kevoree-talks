    ...
    this.startKeynote = function () {
        ...
        promise.then(function () {
            window.addEventListener('resize', function () {
                if (!isListMode()) {
                    self.applyTransform(self.getTransform());
                }
            }, false);
            ...
            if (orderedpluginListeners) {
                for (var i = 0; i < orderedpluginListeners.length; i++) {
                    try {
                        orderedpluginListeners[i].listener.start();
                    } catch (e) {
                        console.error(e.message);
                        console.warn("Unable to execute the method 'start' on ", orderedpluginListeners[i].listener);
                    }
                }
            }
        })
...
//                }
    ...
    this.getCurrentSlideNumber = function () {
        var currentSlideId = self.url.hash.substr(1);
    ...
    function isListMode() {
        return 'slide' !== self.url.search.substr(1);
    }
    ...
    /*function goToSlide(slideNumber) {
     ...
     url.hash = self.getSlideHash(slideNumber);
     if (!isListMode()) {
     ...
     function goToList() {
     if (self.getCurrentSlideNumber() === -1) {
     history.pushState(null, null, url.pathname);
     } else {
     history.pushState(null, null, url.pathname + self.getSlideHash(self.getCurrentSlideNumber()));
     }
     enterListMode();
    ...
    this.getTransform = function () {
        var denominator = Math.max(
            (self.body.clientWidth / window.innerWidth),
            (self.body.clientHeight / window.innerHeight)
        );
        return 'scale(' + (1 / denominator) + ')';
    };

    this.applyTransform = function (transform) {
        self.body.style.WebkitTransform = transform;
        self.body.style.MozTransform = transform;
        self.body.style.msTransform = transform;
        self.body.style.OTransform = transform;
        self.body.style.transform = transform;
    };
